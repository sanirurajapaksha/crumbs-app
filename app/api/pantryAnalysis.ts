...
export async function analyzePantryItemsFromImage(imageUri: string, userId?: string): Promise<AnalysisResult> {
    try {
        // Validate image URI
        if (!imageUri) {
            throw new AIProcessingError(ErrorType.VALIDATION, "Invalid image provided", "Image URI is empty or undefined");
        }
        ...
        if (!base64Image) {
            throw new AIProcessingError(ErrorType.PROCESSING, "Failed to process image", "Could not convert image to base64 format");
        }

        // Analyze with Groq LLaVA model
        const result = await analyzeImageForPantryItems(base64Image);

        // Convert to pantry items (with AI categorization)
        const pantryItems = await convertToPantryItems(result.items, userId);
...
export async function analyzePantryItemsFromAudio(audioUri: string, userId?: string): Promise<AnalysisResult & { transcription?: string }> {
    try {
        // Validate audio URI
        if (!audioUri) {
            throw new AIProcessingError(ErrorType.VALIDATION, "Invalid audio provided", "Audio URI is empty or undefined");
        }
